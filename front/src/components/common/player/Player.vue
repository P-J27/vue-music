  ...
  <div id="Player">
    <el-card v-if="playerMusicId">
      <div class="content-width">
        ...
        <el-image :src="musicDetails.al.picUrl"
                  fit="cover"
                  lazy>
        </el-image>
        <!--
        ...
        <div class="musicText">
          ...
          <p class="singerName">
            <span v-for="(item,index) in musicDetails.ar"
                  :key="index">
        ...
        <!--按钮-->
        <div>
          <span @click="changeMusic('up')"
                class="clickIcon iconfont icon-shangyiqu left"></span>
          <span v-if="isPlayer"
                @click="playOrStopMusic"
                class="clickIcon iconfont icon-icon_bofang stop"></span>
          <span v-else
                @click="playOrStopMusic"
                class="clickIcon iconfont icon-bofang player"></span>
          <span @click="changeMusic('down')"
                class="clickIcon iconfont icon-xiayiqu right"></span>
        </div>
        ...
        <div class="playerTime">
          <span>{{ playerNowTime }} <span class="line"></span>
            {{ playerAllTime }}</span>
        ...
        <div>
          ...
          <span @click="changePlayerMode(1)"
                v-else
                class="clickIcon iconfont icon-liebiaoxunhuan liebiao"></span>
          <!--播放列表-->
          <span @click="showMusicList"
                class="clickIcon iconfont icon-bofangduilie liebiao">
...
export default {
  ...
  methods: {
    ...
    updateTime() {
      ...
      //获取总时间长
      this.playerAllTime = this.$utils.formatSecondTime(
        this.$refs.audioRef.duration
      );
      //获取当前播放时间戳
      this.playerNowTime = this.$utils.formatSecondTime(
    ...
    changeMusic(type) {
      ...
      if (this.playerMode === 0) {
        ...
        //1: 随机播放
        this.playerIdIndex = this.$utils.getRadomIndex(
          0,
          this.musicDetailsList.length
        );
