...
<template>
  <div id="SongListDetails">
    <el-card>
      <div class="top">
        <div class="img">
          <el-image :src="songDetails.detailsImg"
                    fit="cover"></el-image>
        </div>
        <div class="info">
          <h2 class="title">{{songDetails.detailsName}} </h2>
          <div class="author">
            <el-image :src="songDetails.authorImg + '?param=130y130' "
                      fit="cover">
            </el-image>
            <span>{{songDetails.authorName}}</span>
          </div>
          <div class="tag"
               v-if="songDetails.detailsTag.length==0?false:true">
            <span>标签：</span>
            <ul>
              <li v-for="(item,index) in songDetails.detailsTag"
                  :key="index">
                <el-tag effect="plain"
                        type="success"
          ...
          <div class="describe">
            <p ref="describeRef"><span>描述：</span>{{songDetails.detailsInfo}}
            </p>
          </div>
          <span v-if="describeHeight>40"
                @click="showDescribe"
                class="showDescribe">
...
export default {
  ...
  updated() {
    const h = window.getComputedStyle(this.$refs.describeRef).height;
    this.describeHeight = h.slice("px")[0] - 0;
  },
};
</script>
